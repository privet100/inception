...
+ wordpress/wp-config-create.sh 
  ...
  - $table_prefix для определения префикса, который добавляется ко всем таблицам базы данных, создаваемым для сайта WordPress
    * использование стандартного префикса wp_ может также быть уязвимым для атак SQL-инъекций, изменение префикса на что-то более уникальное может повысить безопасность вашего сайта
