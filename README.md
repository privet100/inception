...
* ищет запрашиваемый файл на wp-volume, когда поступает HTTP-запрос, Nginx 
  + если запрошен `/wp-content/uploads/image.jpg`, статический файл, то находит image.jpg в wp-volume и возвращает его клиенту
  + если запрошен `/index.php`, PHP-скрипт, страница с динамическим содержанием, то передаёт обработку контейнеру PHP-FPM, 
* **Dockerfile**                
...
* **Dockerfile**
  ...
  + `--no-cache` устанавливает пакеты без кэша
    - экономится место в образе
  ...
  + `sed -i "s|listen = 127.0.0.1:9000|listen = 9000|g" /etc/php8/php-fpm.d/www.conf`
    ...
    - PHP-FPM будет слушать входящие соединения 127.0.0.1 на порту 9000, что ограничивает доступ только для запросов внутри контейнера
    - WordPress и PHP-FPM взаимодействовать через сокет или IP-интерфейс, который был бы доступен другим контейнерам
  ...
  + в `/var/www` будут помещены файлы WordPress, там вся работа
* **wp-config-create.sh** 
