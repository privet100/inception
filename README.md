...
+ **srcs/nginx/nginx.conf**
  ...
  - SSL-сертификат для обеспечения HTTPS-соединений, закрытый ключ SSL используется с сертификатом
  ...
  - `location` для определения правил, по которым сервер будет обрабатывать запросы к определённым URL или файлам
  - `~` указывает, что после него будет регулярное выражение
  - `\.php$` регулярное выражение, ищет файлы с расширением .php
  - `location /` для запросов на корневой путь без указания файла, пытается найти запрашиваемый файл $uri, если не найден, перенаправляет запрос на index.php, передавая параметры запроса через $args
  - `location ~ \.php$ { ... }` для обработки PHP-файлов и их передачи на сервер FastCGI (например, PHP-FPM), который будет их исполнять, применять это правило ко всем запросам, которые направлены на файлы с расширением .php.
    * вместо того чтобы отдавать файл напрямую, Nginx передаёт его на FastCGI-сервер (обычно это PHP-FPM), который исполняет PHP-код и возвращает результат обратно через Nginx в браузер клиента
    * Nginx не может интерпретировать PHP напрямую
    * важно для работы PHP-приложений на веб-сервере Nginx
