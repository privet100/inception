...
* **Dockerfile**
  ...
  + MariaDB/MySQL/docker.cnf конфиг MariaDB { echo '[mysqld]'; echo 'skip-host-cache'; echo 'skip-name-resolve'; echo 'bind-address=0.0.0.0'; } | tee  /etc/my.cnf.d/docker.cnf;
    ...
    - skip-host-cache отключает кэширование информации о хостах - для избежания проблем с DNS
    - bind-address=0.0.0.0 сервер доступен для всех IP-адресов, что позволяет подключаться к базе данных извне
    - bind-address=0.0.0.0 сервер слушает на всех сетевых интерфейсах (т.е., принимать подключения с любого IP-адреса), сервер принимает запросы извне контейнера
    - skip-name-resolve отключает обратное разрешение имен хостов - улучшает производительность, так как сервер не пытается разрешить IP-адреса в имена хостов
  + `sed -i "s|skip-networking|skip-networking=0|g" /etc/my.cnf.d/mariadb-server.cnf`
    ...
    - skip-networking=0 отключает настройку skip-networking => MariaDB использует сеть для приёма соединений, сервер принимает входящие сетевые соединения, доступ к бд возможен не только через локальный Unix-сокет
  + `mysql_install_db` создает основные структуры данных для MariaDB, инициализирует бд
...
* в docker-compose для MariaDB и WordPress не указаны порты `ports: "3306:3306"`
  + нет пробросв порта 3306 на хост-машину
  ...
  + проброс портов `ports` нужен только для доступа к сервису извне Docker (например, с хост-машины), чтобы подключаться к БД через клиент БД (MySQL Workbench, phpMyAdmin) и вручную управлять БД
...
* логи контейнеров по умолчанию в системные логи Docker, которые можно просмотреть `docker logs <container_name>`
  + для перенаправления логов в файл можно использовать инструкции в docker-compose.yml или Dockerfile:
    ```
    logging:
      driver: "json-file"
      options:
        ...
        max-file: "3"
    ```
  + Если приложение Python использует конкретный лог-фреймворк, логи могут сохраняться в файл, указанный в коде:
    ```
    import logging
    logging.basicConfig(filename='/var/log/inception.log', level=logging.INFO)
    ```
  + если вы используете в Makefile команды для логирования при сборке, они могут записывать логи в определённые файлы
