...
* **docker-compose.yml**
  ...
  + args (build arguments)
    - передаются на этапе сборки образа с помощью Dockerfile, доступны только на этапе сборки контейнера (docker build) для передачи данных
    - не сохраняются в контейнере после сборки
    - применяются в docker-compose:args или Dockerfile:ARG
    - версии зависимостей, конфигурации для сборки, ...
  + environment (переменные окружения)
    - переменные доступны при запуске, на этапе _выполнения_ контейнера, во время работы контейнера
    - ключи доступа, пароли, порты, ...
  + DB_NAME, DB_USER, DB_PASS нужны в процессе выполнения контейнера WordPress
    - wordpress использует их для подключения к базе данных во время работы приложения, когда контейнер запущен, а не на этапе сборки.
    - на этапе сборки эти переменные не обязательны, потому что WordPress сам по себе не требует их для того, чтобы собрать контейнер
+ **.env**
  ```
  DB_NAME=wp
  DB_ROOT=2
