...
+ wordpress/wp-config-create.sh 
  ...
  - $table_prefix = префикс, который добавляется ко всем таблицам базы данных, создаваемым для сайта WordPress
    * использование стандартного `wp_` может также быть уязвимым для атак SQL-инъекций
    * префикс используется в SQL-запросах WordPress для создания и доступа к таблицам базы данных
    * не может быть пустым, если вы оставите его пустым или удалите, WordPress не сможет корректно взаимодействовать с таблицами базы данных
