...
+ **srcs/nginx/nginx.conf**
  ...
  - `location ~ \.php$ { ... }` обрабатывает PHP-файлы через FastCGI (вероятно, через PHP-FPM)
  ...
  - fastcgi_index index.php; index.php = по умолчанию 
  - include fastcgi_params; стандартный набор параметров для FastCGI, который необходим для правильной работы PHP
