...
* в обычной системе nginx
  + после старта отделяется от основного процесса
  + продолжает работу в фоне (демоном)
  + если так сделать в контейнере
    - Docker ожидает, что процесс в контейнере будет основным и управляемым 
    - Docker увидит, что нет активного процесса, за которым можно следить, и заершит контейнер
* в Docker-контейнерах `CMD ["nginx", "-g", "daemon off;"]`
  + nginx не отделяется в фоновый процесс от основного 
  + nginx = основной процесс контейнера, PID 1
  + Docker отслеживает PID 1
  + PID 1 управляет контейнерами
