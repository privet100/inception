...
### DNS
* DNS-сервер
* для создания инфраструктуры контейнеризации 
* настройка DNS, чтобы контейнеры могли правильно взаимодействовать друг с другом по доменным именам, а не по IP-адресам
* использование встроенного механизма Docker для разрешения DNS через сетевые драйверы
  + например драйвер Docker's default bridge network поддерживает автоматическое разрешение DNS для контейнеров
