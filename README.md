...
* **docker-compose.yml**
  ...
  + args (build arguments)
    ...
    - применяются в docker-compose.yml или в Dockerfile через ARG (или нужно и там, и там?)
  + environment (переменные окружения)
    - передаются контейнеру при запуске, на этапе выполнения контейнера
    - остаются доступными во время работы контейнера
