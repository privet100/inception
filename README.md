...
+ docker-compose.yml
  ...
  - `-d` = detached mode = фоновый режим, контейнеры будут запущены в фоновом режиме, а не в привязке к вашему терминалу
+ .env
  ```
  DB_NAME=wp
  DB_ROOT=2
...
+ wordpress/wp-config-create.sh 
  ...
  - ABSPATH
    ...
    * константа, определённая WordPress, которая содержит абсолютный путь к корневой директории установки WordPress
    * обычно задаётся в wp-config.php и указывает на корневую папку сайта
    * определяет корневую директорию WordPress (где файлы ядра WordPress)
    * используется для обращения к файлам, не завися от того, где фактически установлена CMS
    * для работы с конфигурационными файлами и другими ресурсами
    * помогает защитить файлы WordPress от прямого доступа через веб-сервер. Например, если кто-то попытается напрямую открыть wp-config.php, он проверит наличие константы ABSPATH для предотвращения доступа
  ...
  -  wp-config.php запускается на каждом запросе к сайту
    * играет ключевую роль в инициализации и настройке сайта
    * содержит настройки базы данных, секретные ключи, параметры безопасности
    * 1. запускается, когда пользователь отправляет запрос на сайт (например, открывает страницу в браузере), WordPress начинает выполнение скриптов с файла index.php, расположенного в корневой директории сайта. WordPress получает настройки подключения к бд (хост, имя базы данных, имя пользователя, пароль), может подключиться к бд и продолжить обработку запроса пользователя
    * 2. подготавливает настройки для корректной работы системы, из файла index.php WordPress начинает загружать своё ядро. Один из первых шагов в процессе загрузки — это выхов wp-config.php через основной файл wp-load.php
  - require_once ABSPATH . 'wp-settings.php';
    * для подключения файла wp-settings.php
    * require_once для включения и выполнения кода из указанного файла
    * 'wp-settings.php' отвечает за инициализацию ядра WordPress: он загружает необходимые файлы, настраивает среду WordPress, загружает плагины, активирует темы, выполняет другие важные шаги по настройке WordPress

Как это работает вместе:
Конкатенация пути: ABSPATH . 'wp-settings.php' создаёт полный путь к файлу wp-settings.php в корневой директории WordPress.
Подключение файла: С помощью require_once этот файл подключается только один раз, и его код выполняется, обеспечивая загрузку всех необходимых компонентов для работы WordPress.
