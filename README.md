...
+ wordpress/wp-config-create.sh 
  ...
  -  wp-config.php запускается на каждом запросе к сайту
    * инициализация и настройке сайта
    * из index.php WordPress загружает своё ядро
    * один из первых шагов в процессе загрузки — это выхов wp-config.php через wp-load.php
    * 1. подготавливает настройки для корректной работы системы (настройки базы данных, секретные ключи, параметры безопасности)
    ...
    * если в wp-config.php значение DB_NAME = ${DB_NAME}, это placeholder, который должен быть заменён реальным именем базы данных с помощью инструментов развертывания или конфигурационных менеджеров, которые подставляют реальные значения переменных окружения при деплое
  - require_once ABSPATH . 'wp-settings.php';
    * включение и выполнения кода из указанного файла wp-settings.php
    * один из ключевых файлов WordPress, инициализация и настройки системы, инициализация ядра WordPress: загружает необходимые файлы, настраивает глобальные переменные, настраивает среду WordPress, загружает плагины, активирует темы, выполняет другие важные шаги по настройке WordPress
  ...
  - ABSPATH
    ...
    * константа, абсолютный путь к корневой директории WordPress (корневая папка сайта, где файлы ядра WordPress)
    * задаётся в wp-config.php 
