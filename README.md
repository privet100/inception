...
* `location ~* \.(js|css|png|jpg|jpeg|gif|ico|svg)$`
    ...
    - `log_not_found off` отключает запись в лог, если сервер не находит один из этих файлов
* `location ~ \.php$ { ... }` - запрошен PHP-скрипт, страница с динамическим содержанием
    - nginx не интерпретирует PHP напрямую
    - передаёт обработку контейнеру PHP-FPM
    - извлекает из URI имя исполняемого скрипта относительно корня сайта и автоматически присваивает его `$fastcgi_script_name`
