...
* сервисы автоматически подключаются к виртуальной сети
  + внутри сети Docker контейнеры общаюся напрямую по внутренним IP-адресам и портам без необходимости явного проброса портов
...
* проверка после внесения изменений:
  ...
  + проверьте, что ключевые функции вашего веб-приложения работают корректно
    - форма на сайте корректно отправляет данные и обрабатывает их
  + убедитесь, что контейнеры взаимодействуют друг с другом (например Nginx корректно передает запросы на php-fpm)
  + убедитесь, что бд доступна и взаимодействует с приложением, попробуйте подключиться к базе данных из контейнера приложения
* `nslookup akostrik.42.fr` корректно ли браузер разрешает DNS-запросы
  + покажет IP-адрес, к которому разрешается домен
  + если возвращает IP-адрес, то DNS-разрешение работает правильно
  + сравните IP-адрес, возвращаемый nslookup, с IP-адресом, который ваш сервер должен использовать
  + ошибка NXDOMAIN
    - домен akostrik.42.fr должен существовать в DNS-записях на используемом DNS-сервере
    - akostrik.42.fr должен быть зарегистрирован
    - DNS-сервер, который используется (10.0.2.3) должен иметь доступ к домену akostrik.42.fr
    - DNS-сервер, который используется (10.0.2.3) должен поддерживать домен akostrik.42.fr
    - локальный или внутренний сервер DNS-сервер должен иметь глобальный доступ к интернету
    - локальный или внутренний сервер DNS-сервер должен поддерживать разрешение публичных доменов
    - для домена должны быть правильно настроены публичные DNS-записи: Используйте публичные DNS-сервисы, такие как Google DNS или Cloudflare, чтобы проверить, доступен ли домен в глобальных DNS-записях.
Например, команда для проверки через Google DNS:
nslookup akostrik.42.fr 8.8.8.8

Или через Cloudflare DNS:
nslookup akostrik.42.fr 1.1.1.1
Если ответ будет таким же с NXDOMAIN, это означает, что домен действительно не зарегистрирован или неправильно настроен.

Проверить статус домена онлайн:
Используйте сервисы, такие как WHOIS, чтобы проверить, зарегистрирован ли домен akostrik.42.fr и какие DNS-записи ему назначены.
Проверка конфигурации DNS-серверов:

Если вы контролируете DNS-записи для этого домена, проверьте конфигурацию DNS-записей у вашего регистратора доменов и убедитесь, что они правильно настроены для публичного доступа.
Проверьте локальные настройки DNS:

Если вы находитесь в локальной сети с нестандартными DNS-настройками, это может вызвать проблему. Попробуйте изменить DNS-сервер на ваш компьютер на публичный, например, Google (8.8.8.8) или Cloudflare (1.1.1.1).
