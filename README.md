...
### DNS-сервер, настройка DNS
* чтобы контейнеры взаимодействовали друг с другом по доменным именам, а не по IP-адресам
* если в docker-compose не указана сеть, по умолчанию контейнеры подключаются к Docker's default bridge network — стандартной сети Docker для изолированных контейнеров
  + контейнеры находястя в одной сети и взаимодействуют через DNS-имена, соответствующие их именам контейнеров
...
* `nslookup akostrik.42.fr` корректно ли браузер разрешает DNS-запросы
  ...
  + если ошибка NXDOMAIN
    ...
    - используемый DNS-сервер (10.0.2.3) должен иметь доступ к домену akostrik.42.fr
    - используемый DNS-сервер (10.0.2.3) должен поддерживать домен akostrik.42.fr
    - локальный или внутренний DNS-сервер должен иметь глобальный доступ к интернету
    - локальный или внутренний DNS-сервер должен поддерживать разрешение публичных доменов
    - `nslookup akostrik.42.fr 8.8.8.8` проверка через Google DNS, доступен ли домен в глобальных DNS-записях
    - `nslookup akostrik.42.fr 1.1.1.1` проверка через Cloudflare DNS, доступен ли домен в глобальных DNS-записях
    - whois: проверить статус домена, зарегистрирован ли домен akostrik.42.fr и какие DNS-записи ему назначены, 
    - конфигурация DNS-серверов: если вы контролируете DNS-записи для этого домена, проверьте конфигурацию DNS-записей у вашего регистратора доменов и убедитесь, что они правильно настроены для публичного доступа
    - локальные настройки DNS: если вы в локальной сети с нестандартными DNS-настройками, это может вызвать проблему. Попробуйте изменить DNS-сервер на ваш компьютер на публичный, например, Google (8.8.8.8) или Cloudflare (1.1.1.1)
